...
use crate::terrain::{Chunk, ChunkBounds, Terrain};
...
use super::{Block, BlockVertex, VoxelChunk, ChunkMesh, VoxelTerrain, CHUNK_SIZE};
...
impl VoxelTerrain {
    pub fn new() -> Self {
        ...
        let origin = VoxelChunk::new((0.0, 0.0, 0.0));
        ...
        Self {
            chunks: HashMap::<ChunkBounds, VoxelChunk>::new(),
            chunk_receiver: rx,
            shader,
            grass_texture,
            stone_texture,
...
impl Terrain for VoxelTerrain {
    ...
    fn process_line(&mut self, line: Option<(Line, glfw::MouseButton)>) {
...
fn chunkloader(radius: i32, x_dir: i32, z_dir: i32, tx: mpsc::Sender<VoxelChunk>) {
    let mut x: i32 = 1;
    let mut z: i32 = 0;
    ...
    loop {
        ...
        let new_chunk: VoxelChunk;
        if z_dir > 0 {
            new_chunk = VoxelChunk::new(((x * x_dir) as f32, 0.0, z as f32));
        } else {
            new_chunk = VoxelChunk::new(((z * z_dir) as f32, 0.0, (x * x_dir) as f32));
        }
